    ...
    <form onSubmit={handleSubmit} className="w-full max-w-xl">
      <div className="bg-white p-2 rounded-2xl border border-slate-300 shadow-sm flex flex-col sm:flex-row items-center gap-2 focus-within:border-[#1264D6] focus-within:ring-2 focus-within:ring-[#1264D6]/20 transition-all">
        ...
        <div className="relative flex-1 w-full flex items-center min-h-[44px]">
          ...
          <input
            type="text"
            placeholder="Enter your location (e.g. Delhi, Noida, Surat)"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            className="w-full py-2.5 pr-2 text-base sm:text-sm text-[#09265C] font-semibold bg-transparent border-none outline-none placeholder:text-slate-400 placeholder:font-normal"
          />
          <button
            type="button"
            onClick={handleDetectGPS}
            title="Detect GPS location"
            className="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-[#1264D6] transition-colors shrink-0 rounded-lg hover:bg-blue-50 active:scale-95"
          >
            {isLoading ? (
        ...
        <button
          type="submit"
          className="w-full sm:w-auto px-6 py-3.5 sm:py-3 min-h-[48px] bg-[#1264D6] hover:bg-blue-700 text-white font-bold text-sm sm:text-sm rounded-xl shadow-xs transition-all flex items-center justify-center shrink-0 tracking-wide active:scale-98"
        >
          Find a Worker
        </button>
      </div>
